...
function DropImage() {
  const acceptedFileTypes = ['video/mp4', 'video/mov', 'video/avi'];
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    if (validFiles.length === 0) {
      setError('Invalid File, please select a valid file (mp4 / mov / avi)');
      setDisable(true);
    } else {
      setError('');
      setDisable(false);
  ...
  const submitImage = async () => {
    if (images.length === 0) {
        setError('No files you selected')
        return ;
    }
      ...
      <Grid container spacing={2}>
        {images?.map((item, i) => (
          <Grid key={i} item xs={12}>
            <Box w={'100%'} sx={{ position: 'relative' , display:'flex', alignItems:'center', justifyContent:'center'}}>
